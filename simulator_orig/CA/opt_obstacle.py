...
#obstacleの初期値(最大100個のobject)
a = conf["width"]*conf["height"]
while a>100:
    obst_list = []
    obst_list =\
        np.array([int(random.random()*1.2) for i in range(625)])
    a = np.sum(obst_list)
#obstacleのリストから座標に変換
def make_obst_xy_list(obst_list):
    # 1を超えたものを抽出
    xy_list = []
    indexes = [i for i, x in enumerate(list(obst_list)) if x >= 1]
    for index in indexes:
        x = (index+1) % conf["width"]
        if x != 0:
            y = (index+1) / conf["width"] + 1
        else:
            y = (index+1) / conf["width"]
        xy_list.append((x,y))
    return xy_list
...
def initialize_cell(obst_xys):
    ...
    for obst_xy in obst_xys:
        cell[obst_xy[1]][obst_xy[0]] = Cell("obstacle")
...
def run(obst):
    ...
    steps = []
    #obstacleの配置
    obst_xys = make_obst_xy_list(obst)
    for i in range(1):
        step = 0
        # セルの初期化とSFFの設定
        cell = initialize_cell(obst_xys)
        SFF = SFF_Euclid(cell)
        while len(particles) != 0 and step < 1000:
