...
config = {
    "width" : 250,                  #corridorの幅
    "height" : 250,                 #corridorの高さ
    ...
    "rotate_thetas" :                #衝突時の回転する方向
        np.array([np.pi/60,-np.pi/60,-np.pi/2,-np.pi/3,-np.pi/6,
...
class Pedestrians(object):
    def __init__(self,id,start,goal,kind):
        ...
        #ゆっくりさん(kind == 1)
            ...
            self.desired_v = np.random.normal(config["ave_desired_v"], config["var_desired_v"])/2
    ...
    #退出処理
    def exit(self):
        if np.linalg.norm(self.x-self.goal)<config["repulsive_r"]*2:
            if(self.isexit()):
                self.isvalid = False
...
def run():
    ...
    for i in range(config["ped_num"]):
        if i < 50:
            ped = Pedestrians(i,
                              np.array([np.random.uniform(0,config["width"]),np.random.uniform(0,config["height"])]),
                              goal,
                              0)
            #被らないようにランダム生成し直す
    ...
    while(flags):
        print step, len(peds)
        frame = record_data(step,peds,frame)
        screen.fill((0,0,0))
        peds = update(peds)
        #全員退出したら終了
        if len(peds) == 0:
