    ...
    def dest_update(self,p):
        ...
        if not now.exit:
            ...
            if self.is_inside(now.dest_range,self.x):
                ...
                if _p < p and self.destnow<len(self.destorder):
                    self.destnow += 1
...
def update(props,peds,timescale):
    ...
    for i,ped in enumerate(peds_tmp):
        ...
        if ped.isvalid:
            ...
            peds[i].v_update()
            peds[i].x_update(timescale)
            peds[i].dest_update(0.1)
